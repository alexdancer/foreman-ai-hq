...
- Card audit evidence is no longer inlined. Selecting a card opens an **Evidence Drawer** that shares the `frontend/src/components/SessionEvidence.jsx` rendering boundary with the Session Report; `/sessions/{id}` remains the permalink and full audit view. Drawer data is fetched on open rather than inlined into the Floor payload.
