...
- Needs You is **project-scoped**, not global: a decision is always read in the context of one project's repo, adapter, and budget. The Dashboard now rolls up only the counts emitted by each project-scoped projection; this is a display, not a second queue. The Dashboard contract is owned by `docs/design/portal-operator-workbench-spec.md`.
