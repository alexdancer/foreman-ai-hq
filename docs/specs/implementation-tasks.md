# Foreman implementation and task catalog

## How to use this catalog

This is the versioned task plan consumed by `/to-tickets` and `/implement`. Each item is a tracer-bullet slice or a deliberate wide refactor. Publish implementation issues to GitHub in dependency order with `ready-for-agent`; do not create a single horizontal layer ticket. The catalog is planning-only for this migration: all items below are future work and remain unchecked until implementation and the test contract pass.

## Issue template

## What to build

Describe the complete user-visible or verifiable behavior delivered by the slice, not a layer-by-layer checklist.

## Acceptance criteria

- [ ] The relevant product and technical specification is satisfied.
- [ ] The matching test-contract seam passes.
- [ ] Evidence and provenance remain visible where required.

## Blocked by

List only genuine prerequisite issue numbers, or `None — can start immediately`.

## Accepted Portal workbench slices
...
- Shared tokens/primitives precede shell, review, Pipeline, Floor, settings, and evidence restyling.
- Needs You promotion and Planning Inbox removal land together.
- Presentation work must not alter backend contracts, projections, mutation paths, or task lifecycle semantics.
- Verification includes frontend checks, focused browser/render tests, repository workflow checks, and proportional documentation-link checks.
