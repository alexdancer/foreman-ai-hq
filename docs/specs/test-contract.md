# Foreman test contract

## Contract purpose

Tests prove external behavior at agreed public seams. This contract consolidates every scenario from the former capability documents and adds the accepted Portal design and workflow migration checks.

## Test seams

- FastAPI route and service seams for authentication, projections, mutations, Worker Runs, budget governance, setup, and reports.
- React render/interaction seams for shell routing, Task Breakdown Review, Pipeline, Needs You, Floor, Drawer, settings, and evidence.
- CLI seams for `foremanctl` installation, init, serve, check, seed-demo, and diagnostics.
- Isolated Recorded Demo Run seam using synthetic providers and temporary repositories.
- Repository workflow seam for skill discovery, artifact coverage, labels, and absence of retired workflow files.
...
## Accepted Portal design checks

- Task Breakdown Review exposes focus, selection, edited/incomplete state, independent zone scrolling, keyboard navigation, explicit accepted-task confirmation, and disabled-reason text.
- Pipeline exposes next required action, stage counts/filter semantics, one ledger over all four authoritative buckets, row evidence, and no Planning Inbox duplicate.
- Needs You is direct-loadable at the canonical project route and preserves inline manual-estimate behavior.
- Shell exposes grouped navigation rail, project switcher, context bar, active state, badges, logout/recovery behavior, and back/forward navigation.
- Shared primitives expose focus rings, glyph-plus-label statuses, reduced-motion fallback, full-width selects, and no nested panel styling.
- Evidence Drawer preserves focus trap, Escape, opener restore, live refresh, and provenance beside estimate-versus-actual figures.

## Migration verification checks

- Exactly nine project-local skills resolve for Pi, Codex, and Claude Code; canonical bodies exist only under `.agents/skills` and portable links resolve.
- Required GitHub labels exist in `alexdancer/foreman-ai-hq`.
- Product, technical, test, and task artifacts plus the migration ledger cover every inventory row.
- Case-insensitive repository search excluding `.git` finds no retired workflow name, command, configuration, dependency, or path.
