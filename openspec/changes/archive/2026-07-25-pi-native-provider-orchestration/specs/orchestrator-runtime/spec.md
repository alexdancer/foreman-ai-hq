...
**Reason**: The planning Orchestrator now runs on its own configured model provider, not the Harness Proxy, so a proxy custom-provider profile is no longer the transport. Provider selection is native pi config.
**Migration**: Replace the proxy-only `models.json` override with the operator's own pi provider config; the Harness Proxy remains only for `proxy_governed` Worker adapters.

### Requirement: Governed pi launch injects the planning bearer at launch
**Reason**: With no proxied turn, there is no per-session proxy bearer to inject; pi authenticates to its own provider from the operator's pi auth, and spend is accounted from native usage.
**Migration**: Remove planning-bearer minting/injection from the launch path; account planning spend from pi's native usage evidence instead.
